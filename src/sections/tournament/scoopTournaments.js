...
export default function ScoopTournaments() {
    const { user } = useAuth();
    ...
    const { sendMessage, readyState } = useWebSocket(SERVER_WSS_ADDR, {
        onOpen: () => {
            ...
            sendMessage(JSON.stringify({ command: 'get-scoop-list' }));
        },
        onMessage: (evt) => {
            ...
            try {
                ...
                if (data && data.list && data.command === 'get-scoop-list') {
                    setRooms(data.list);
                }
            }
            catch (err) {
    ...
    return (
        <Container>
            <div className="flex gap-1 justify-end items-center p-8">
                {user?.role == 1 && <Button variant={'outlined'} onClick = {()=>{openDialog()}}>Create Scoop</Button>}
            </div>
            ...
            {rooms.length === 0 &&
                <div className="flex w-full justify-center p-4 gap-4 flex-col items-center">
                    <Typography>No Filtered Tournaments</Typography>
                </div>
            }
