...
export default function WcoopTournaments() {
    const { user } = useAuth();
    ...
    const { sendMessage, readyState } = useWebSocket(SERVER_WSS_ADDR, {
        onOpen: () => {
            ...
            sendMessage(JSON.stringify({ command: 'get-wcoop-list' }));
        },
        onMessage: (evt) => {
            ...
            try {
                ...
                if (data && data.list && data.command === 'get-wcoop-list') {
                    setRooms(data.list);
                }
            }
            catch (err) {
    ...
    return (
        <Container>
            <div className="flex gap-1 justify-end items-center p-8">
                {user?.role == 1 && <Button variant={'outlined'} onClick = {()=>{openDialog()}}>Create Wcoop</Button>}
            </div>
